	...
	public function build(?SessionStoreInterface $sessionStore = null): Server
	{
		$builder = Server::builder()
			...
			->setInstructions(
				...
				. '(songs, exercises, techniques). Every lecture and song has a fixed learning status: '
				. '"To Learn", "Learning" or "Mastered". '
				. 'Typical flow when adding lectures from an external source: '
				. '1) call find_course_by_name to check if the target course exists; '
				. '2) if it does not, call create_course; '
				. '3) call create_lecture for each item (defaults to the "To Learn" status). '
				. 'Typical flow when practising a lecture: '
				. '1) call find_lecture_by_name to locate the lecture; '
				. '2) call move_lecture with status="Learning" to start practising it; '
				. '3) once it is mastered, call move_lecture with status="Mastered". '
				. 'Songs (create_song) are workspace-level items with author/album/cover metadata; a song can be '
				. 'attached to a course (add_song_to_course) where it appears on the board like a lecture. '
				. 'Lectures and songs also support tags, file attachments, watchers, and an audit log (list_events). '
