 ...
 * on invalid alphaTex `valid` is false and `errors` carries the tab-service validation errors so the
 * agent can fix and retry (instead of receiving an opaque tool error).
 */
final readonly class McpTabResultDto
{
	/** @param list<array{message: string, line: ?int, col: ?int, offset: ?int}> $errors */
	public function __construct(public bool $valid, public ?McpTabDto $tab, public array $errors = [],)
	{
	}

	public static function success(McpTabDto $tab): self
	{
