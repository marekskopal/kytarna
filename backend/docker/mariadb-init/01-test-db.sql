...
-- create + truncate `kytarna_test`. This script only runs on the very first
-- DB startup (i.e. when the data volume is empty), which is exactly the CI
-- shape.
CREATE DATABASE IF NOT EXISTS `kytarna_test` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
GRANT ALL PRIVILEGES ON `kytarna_test`.* TO 'kytarna'@'%';
FLUSH PRIVILEGES;
